...
export function AwardedNumbersManager({ raffle, onUpdate }: AwardedNumbersManagerProps) {
  ...
  const originalNumbers = useMemo(() => 
    raffle.awardedNumbers ? [...raffle.awardedNumbers].sort((a, b) => a - b) : [], 
  ...
  const handleAddNumber = () => {
    ...
    if (parsedNumber < raffle.minNumber || parsedNumber > raffle.maxNumber) {
      setError(`El número debe estar entre ${raffle.minNumber} y ${raffle.maxNumber}`);
      return;
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const updatedRaffle = await updateAwardedNumbers(raffle._id, sortedNumbers);
  ...
  return (
    <div className="bg-white p-5 rounded-lg shadow-md">
      <h3 className="text-lg font-semibold mb-4">Configurar Números Premiados</h3>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedNumbers.sort((a, b) => a - b).map(num => (
              <div key={num} className="flex items-center bg-gray-100 px-3 py-1 rounded">
